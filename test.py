...
"""
이 파일은 코드를 이해하는 목적이 큽니다. 실제로 app.py에 작성해야합니다.
"""

model = load_pretrained_model("ir_50")  # 모델을 로드합니다.
device = torch.device("cpu")  # nvidia-smi로 할당된 디바이스를 보고 cuda:{번호} 변경
...
def profile_embedding(image):
    ...
    # 업로드된 파일의 원래 이름을 사용하여 임베딩 파일 이름을 설정
    # 파일 확장자를 변경하기 위해 원래 파일 이름에서 확장자를 제거하고 '.pt'를 추가
    # test용으로 filename 넣음
...
def get_users_with_image(images, user_ids):
    ...
    similarity_scores = torch.mm(
        uploaded_features, profile_embeddings.T
...
if __name__ == "__main__":
    if sys.argv[1] == "profile_embedding":
        ...
        profile_embedding(image)  # 이거 하면 face_alighment/profile_images에 embedding pt 생김

    if sys.argv[1] == "get_users_with_image":
        images = [
            "face_alignment/test_images/a2.jpg",
            "face_alignment/test_images/a3.jpg",
            "face_alignment/test_images/a4.jpg",
            "face_alignment/test_images/c1.jpg",
            "face_alignment/test_images/d2.jpg",
        ]
        user_ids = [
            "face_alignment/profile_images/a1.pt",
            "face_alignment/profile_images/c2.pt",
            "face_alignment/profile_images/d1.pt",
        ]
